from emna_tc_ml_tf import emna_tc_ml_tf
import functions
import pickle
import numpy as np
...
for run in range(runs):
    for fun_num in range(1, 29):
        threshold, best_f = emna_tc_ml_tf(obj_function, dim, max_evals, popsize, 100, 3, model)
        if run == 0:
...
pickle.dump(fits, open("pickles/results_TF4_v1_5p_fits.p", "wb"), protocol=4)
pickle.dump(thresholds, open("pickles/results_TF4_v1_5p_thresholds.p", "wb"), protocol=4)
print('fitnesses:')
print(fits)